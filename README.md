...
工具列的「從 EE-Class 下載」可開啟 OUK EE-Class 影片頁面。輸入形如
`https://<學校>.ouk.edu.tw/media/doc/<編號>` 的網址，在內嵌頁面完成學校原有的
登入、MFA 或驗證碼。頁面載入後，應用程式會從網站提供的媒體資料偵測可信任的
直接 MP4，並自動選擇最高解析度來源；不需要先在頁面中播放影片。找到來源後會
啟用「下載影片」，選擇永久儲存位置後，程式會下載影片並自動建立影格索引。

Qt WebEngine 可能因缺少網站影片使用的編解碼器而在內嵌播放器顯示
`player-noneSupport`；這不影響應用程式偵測及下載頁面提供的直接 MP4。若狀態列
顯示找不到可下載的直接 MP4，請確認已登入且帳號有權限觀看該內容。

登入 Cookie 由獨立的 Qt WebEngine profile 保存，以便下次啟動沿用登入狀態；
應用程式不接收或保存帳號密碼。可在下載視窗按「清除登入資料」刪除 Cookie 與
網頁快取。下載中的檔案先寫入同目錄的暫存檔，完成後才取代目的檔；取消或失敗
會清除暫存檔，不會破壞既有影片。專案檔只記錄下載後的本機影片路徑與指紋，不
保存 Cookie、Authorization header 或帶權杖的媒體網址。

第一版只支援 `ouk.edu.tw` 及其子網域中的 EE-Class 頁面與直接 MP4 媒體，
不支援 HLS (`.m3u8`)、DRM、批次下載或其他站台。只能下載帳號本來就有權限觀看
的內容；若登入已過期、網站改用其他媒體格式或請求被重新導向站外，程式會停止
下載並顯示錯誤。
...
介面預設跟隨系統的淺色／深色外觀；在系統設定切換主題時，視窗會即時更新，不需重新啟動。工具列右側的「外觀」可改為固定淺色或深色，選擇會保留到下次啟動。

1. **開啟影片**：可開啟本機影片，或從 EE-Class 登入並下載 MP4。完成索引後可播放／暫停、調整音量或靜音。暫停與逐幀預覽採用精確影格時間戳，避免可變影格率影片用平均 FPS 推算而偏移。
2. **分析畫面**：按上方工具列的「分析畫面變化」，以 `hh:mm:ss` 設定起始／結束時間點後開始分析。起訖時間都包含在分析內，預設從 `00:00:00` 到影片結尾；區間內每個相鄰影格都會比對，只縮小分析用影像，不跳幀。完成後確認是否套用候選；手動標記、人工修改過的自動標記及區間外既有標記會保留。
3. **調整標記**：點擊時間軸定位，新增標記；可拖曳標記、輸入時間後移動，或在右側多選刪除。同一影格不允許重複標記。清單勾選框決定是否納入匯出；右側欄完整用於顯示與操作標記。
4. **設定裁切**：雙擊標記，拖拉框線或八個把手，也可輸入左／上／右／下百分比。右側預覽顯示最後拉伸效果；可選擇套用至目前標記、清單中選取的標記或全部標記，按「套用」確認，按「取消」保留原設定。
...
| 起始時間 | 00:00:00 | 以 `hh:mm:ss` 設定；僅分析此時間點之後的影格，包含指定時間點 |
| 結束時間 | 影片結尾 | 以 `hh:mm:ss` 設定；僅分析此時間點之前的影格，包含指定時間點 |
