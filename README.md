...
5. **魔術截圖**：先拖出涵蓋課程文本的粗略範圍，再按「魔術截圖」（Ctrl + D），程式會在該範圍內偵測文字並把裁切框縮到文字的最小外接矩形。偵測只在目前框內進行，永遠不會向外擴張；連按兩次結果相同，要重新偵測請先手動調整粗略範圍或按「重設全圖」。「留白」可為結果四周加上邊界；「維持匯出比例」會把框擴張到匯出設定的長寬比，避免緊貼文字的框在匯出拉伸時變形，粗略範圍不夠大時只會盡量逼近。結果只是提案，仍可拖曳微調或按「取消」放棄。套用至多個標記時，每個標記會各自解碼自己的影格、各自偵測，狀態列顯示進度且可取消；取消不會留下半套結果，完成後可一次復原。
...
7. **匯出**：按上方工具列的「匯出」，在對話框選擇 JPG 或 PPTX、全批共用的寬高、JPEG 品質與 PPTX 時間戳。可選擇只匯出清單中目前選取的標記；仍以已勾選納入匯出的項目為準，依影片時間排序。
