...
若頁面附有 EE-Class 時間軸索引，程式也會依網頁提供的毫秒時間自動建立帶標題
的標記；多個索引落在同一影格時保留網頁中的第一筆。這些標記可照常調整、裁切
及匯出，重新分析時不會被取代，儲存專案後也會保留標題。
...
4. **設定裁切**：雙擊標記，拖拉框線或八個把手，也可輸入左／上／右／下百分比。右側預覽依匯出設定顯示最後效果：拉伸時為匯出尺寸的比例，不拉伸時為裁切框本身的比例；可選擇套用至目前標記、清單中選取的標記或全部標記，按「套用」確認，按「取消」保留原設定。
5. **魔術截圖**：先拖出涵蓋課程文本的粗略範圍，再按「魔術截圖」（Ctrl + D），程式會在該範圍內偵測文字並把裁切框縮到文字的最小外接矩形。偵測只在目前框內進行，永遠不會向外擴張；連按兩次結果相同，要重新偵測請先手動調整粗略範圍或按「重設全圖」。「留白」可為結果四周加上邊界；「維持匯出比例」會把框擴張到匯出設定的長寬比，避免緊貼文字的框在匯出拉伸時變形，粗略範圍不夠大時只會盡量逼近；匯出設為「裁切後不拉伸」時沒有變形可避，此選項會自動關閉。結果只是提案，仍可拖曳微調或按「取消」放棄。套用至多個標記時，每個標記會各自解碼自己的影格、各自偵測，狀態列顯示進度且可取消；取消不會留下半套結果，完成後可一次復原。

   魔術截圖同樣只做視覺偵測，**不辨識文字內容、不是 OCR**；它找的是像文字筆劃的小區塊，因此照片、圖表與色塊會被排除，但以大面積插圖為主、文字很少的投影片框會偏鬆，必要時請手動調整。
6. **儲存專案**：保存影片參照、標記、裁切及分析／輸出設定。影片本身不內嵌；搬移影片後可重新指定同一支影片，內容不同會拒絕載入舊影格參照。
7. **匯出**：按上方工具列的「匯出」，在對話框選擇 JPG 或 PPTX、全批共用的寬高、JPEG 品質與 PPTX 時間戳。勾選「裁切後不拉伸（輸出原始像素）」則改成依每個標記自己的裁切像素輸出，此時寬高設定停用。可選擇只匯出清單中目前選取的標記；仍以已勾選納入匯出的項目為準，依影片時間排序。
...
- 輸出模式有兩種，在匯出對話框切換，設定會隨專案保存；舊專案沒有該欄位時視為拉伸，行為與舊版相同。
  - **拉伸（預設）**：裁切後直接拉伸成指定尺寸，預設 1920 × 1080；全批尺寸一致，但裁切框比例不同時畫面會變形。
  - **裁切後不拉伸**：直接輸出裁切區域的原始像素，完全不縮放、不變形、不補留白；寬高設定停用（值仍保留，取消勾選即恢復），每個標記的裁切框不同時每張圖大小也不同。
- JPEG 品質預設 95，兩種模式通用。
...
- 一份 PPTX 只有一種投影片尺寸，因此不拉伸模式下投影片依最大的圖片決定，所有圖片共用同一個縮放比例後置中；因此圖片間的相對大小會保留，較小的裁切框在投影片上也較小。
...
- 魔術截圖是文字**區域**偵測，不辨識文字內容，也不會向外擴張超出你設定的粗略範圍。以大面積插圖為主、文字很少的投影片框會偏鬆；攝影機實拍、捲動或動畫畫面仍可能需要手動調整。
